...
module.exports = class TorrentList extends React.Component {
  render() {
    ...
    const torrentElems = state.saved.torrents.map(torrentSummary =>
      this.renderTorrent(torrentSummary),
    );
  ...
  // <div key="torrent-search" className="torrent-search">
  //         <TextField fullWidth="true" onChange={this.onSearch} />
  //       </div>
  ...
  onSearch(e, value) {
    ...
    // TorrentSearchApi.search("1080", "Movies", 20).then(response => {
  ...
  renderTorrent(torrentSummary) {
    ...
    return (
      <div
        ...
        onContextMenu={
          infoHash && dispatcher("openTorrentContextMenu", infoHash)
        }
  ...
  renderTorrentMetadata(torrentSummary) {
    ...
    function renderDownloadCheckbox() {
      const infoHash = torrentSummary.infoHash;
      const isActive = ["downloading", "seeding"].includes(
        torrentSummary.status,
      );
      return (
        <Checkbox
          key="download-button"
    ...
    function renderSpeeds() {
      let str = "";
      if (prog.downloadSpeed > 0)
        str += " ↓ " + prettyBytes(prog.downloadSpeed) + "/s";
      if (prog.uploadSpeed > 0)
        str += " ↑ " + prettyBytes(prog.uploadSpeed) + "/s";
      if (str === "") return;
  ...
  renderTorrentDetails(torrentSummary) {
    ...
    if (torrentSummary.error || !torrentSummary.files) {
      ...
      if (torrentSummary.error === "path-missing") {
        ...
        message =
          "Missing path: " + TorrentSummary.getFileOrFolder(torrentSummary);
      ...
      const fileRows = torrentSummary.files
        ...
        .map(object =>
          this.renderFileRow(torrentSummary, object.file, object.index),
        );
  ...
  renderFileRow(torrentSummary, file, index) {
    ...
    if (file.currentTime) {
      ...
      positionElem = this.renderRadialProgressBar(
